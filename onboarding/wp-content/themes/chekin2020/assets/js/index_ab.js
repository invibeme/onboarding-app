...
// added on 24-04-2024 Noray On-Premise END
  ...
  jQuery(".erf-reg-form-container input.name").on("keyup change", function (e) {
    jQuery('.hubspot-signup input[name="firstname"]')
      .val(jQuery(this).val())
      .trigger("change");
  });
  jQuery(".erf-reg-form-container input[name='text-gE8dLd']").on(
    "keyup change",
    function (e) {
      jQuery('.hubspot-signup input[name="email"]')
        .val(jQuery(this).val())
        .trigger("change");
    }
  );
  jQuery(".erf-reg-form-container input[name='text-1hJwzr']").on(
    "keyup change",
    function (e) {
      //jQuery('.hubspot-signup input[name="contrasena_onboarding"]').val(jQuery(this).val()).trigger('change');
    }
  );
  jQuery(".erf-reg-form-container input.phone").on(
    "keyup change",
    function (e) {
      jQuery('.hubspot-signup input[name="phone"]')
        .val(
          jQuery(
  ...
  jQuery(".erf-reg-form-container .your-reference").on("change", function (e) {
    jQuery('.hubspot-signup [name="como_nos_conociste_"]')
      .val(jQuery(this).val())
      .trigger("change");
  });
  ...
  jQuery('.erf-reg-form-container .radio input[type="radio"]').on(
    ...
    function (e) {
      ...
      console.log(val);
      jQuery('.hubspot-signup input[name="using_pms"]').prop("checked", false);

      if (val == "yes") {
        jQuery('.hubspot-signup input[name="using_pms"][value="Yes"]').prop(
          "checked",
          true
        );

        jQuery(
          '.hubspot-signup input[name="would_you_like_a_free_bookiply_account_"][value="Have PMS"]'
        ).prop("checked", true);
      } else {
        jQuery('.hubspot-signup input[name="using_pms"][value="No"]').prop(
          "checked",
          true
        );
        //jQuery('.hubspot-signup [name="integracion_pms"][value=""]').prop('checked', true);
  ...
  jQuery(".erf-reg-form-container .your-pms").on("change", function (e) {
    ...
    //jQuery('.hubspot-signup [name="integracion_pms"]').prop('checked', false);
    jQuery('[name="pms_actualizado"]').val(val);

    /*if(jQuery('.hubspot-signup [name="integracion_pms"][value="'+val+'"]').length){
			jQuery('.hubspot-signup [name="integracion_pms"][value="'+val+'"]').prop('checked', true);
		}else{
			jQuery('.hubspot-signup [name="integracion_pms"][value=""]').prop('checked', true);
		}*/
  });
  ...
  jQuery(".erf-reg-form-container .your-property-type").on(
    ...
    function (e) {
      ...
      jQuery('.hubspot-signup [name="property_type"]').val(hbVal).trigger("change");
      /*
		jQuery('.hubspot-signup [name="property_type"] option').attr('selected','');
		jQuery('.hubspot-signup [name="property_type"] option[value=Other]').attr('selected','selected');*/
    }
  );
  jQuery(".erf-reg-form-container .your-room-number-input").on(
    "keyup change",
    function (e) {
      jQuery('.hubspot-signup input[name="number_of_units"]')
        .val(jQuery(this).val())
        .trigger("change");
    }
  );
  ...
  jQuery(".bookiply-account-form input[type=radio]").on("change", function (e) {
    ...
    jQuery(
      '.hubspot-signup input[name="would_you_like_a_free_bookiply_account_"]'
    ).prop("checked", false);

    if (val == "Yes") {
      jQuery(
        '.hubspot-signup input[name="would_you_like_a_free_bookiply_account_"][value="Yes"]'
      ).prop("checked", true);
    } else {
      jQuery(
        '.hubspot-signup input[name="would_you_like_a_free_bookiply_account_"][value="No"]'
      ).prop("checked", true);
      //jQuery('.hubspot-signup [name="integracion_pms"][value=""]').prop('checked', true);
      //jQuery('[name="pms_actualizado"]').val("");
    }
  ...
  //jQuery('.hubspot-signup input[type="submit"]').trigger('click');
    ...
    if (customSyncForm.logo) {
      jQuery(".has-form.v1 .erf-style-rounded-corner .erf-form-html").prepend(
        '<h2 class="has-text-align-center current-urlpms-cont"><img src="' +
          customSyncForm.logo +
          '" class="pms-logo" alt="" > <span class="d-inline-block sync_texttop">Create account in Chekin</span></h2>'
      );
      pmsContent +=
        '<img src="' + customSyncForm.logo + '" class="pms-logo" alt="" >';
    }
	...
	if(yourAddress==''){
      ...
      if (1) {
        if (regError.length == 0) {
          ...
          jQuery('.erf-page.erf-page-2 [class*="field"]').addClass('screen-reader-text').hide(); // opened on 04-03-2024 to make 2 step
          //jQuery('.erf-page.erf-page-1 [class*="field"]').addClass("screen-reader-text").hide(); // commented on 04-03-2024
          if (
            yourPMS_selected == "" ||
            yourPMS_selected == "I don't use a PMS" ||
            yourPMS_selected == "Others PMS"
  ...
  jQuery("body").on("click", ".btn-oauth", function () {
    console.log("submit hb form and do oAuth");
    jQuery('.hubspot-signup input[type="submit"]').trigger("click");
          ...
          success: function (msg) {
            ...
            if (msg.status == false) {
              ...
              if (are_tokens_refetched) {
                ...
                console.log("submit hb form and login");
                jQuery('.hubspot-signup input[type="submit"]').trigger("click");
