<?php if(!is_user_logged_in()) : ?>
    ...
    <div class="erf-password-lost-container <?php echo 'erf-layout'.$attr['layout'].' erf-style-'.$attr['field_style'].' erf-label-'.$attr['label_position']; ?>"  style="display:none">
                    ...
                    <h2><?php _e( 'Password recovery', 'erforms' ); ?></h2>
                <p class="mb-0">
                    <label for="erf_user_login"><?php
                        _e("Your recovery email",'erforms');
                    ?></label>
                </p>
                <form id="lostpasswordform" class="erf-form erf-reset-password-form" action="" method="post" onkeypress="return event.keyCode != 13;">
                    <div class="fb-text form-group pt-0">
                        <input type="email" name="user_login" class="form-control" id="erf_user_login" placeholder="<?php _e( 'Enter recovery email', 'erforms' ); ?>">
                    </div>
                    <div class="erf-error"></div>
                    <div class="fb-text form-group">
                        <button type="button" name="submit" class="lostpassword-button erf-reset-password btn btn-default">
                            <?php _e( 'Send Recovery Link', 'erforms' );?>
                        </button>
                    </div>
                    <input type="hidden" name="action" value="erf_lost_password" />
                    <div class="erf-account-switch erf-clearfix">
                            <a class="erf-show-login" href="javascript:void(0)"><?php _e('Back','erforms'); ?></a>
                    </div>
                </form>
    </div>
