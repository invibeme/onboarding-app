...
if ($options['allow_login_from'] == 'username') {
   // $username_label = __('Username', 'erforms');
    ...
    $username_plaeholder = __('Enter your email', 'erforms');
        ...
        <?php if (!is_user_logged_in()) : ?>
            <form action="" method="post" class="erf-login-form erf-form">
                <div class="fb-text form-group">
                  ...
                  <?php /* ?>  <input required="" placeholder="<?php echo $attr['label_position'] == 'no-label' ? $username_label : ''; ?>" value="" type="text" class="form-control" id="erf_username" name="erf_username"> <?php */ ?>
					
					<input required="" placeholder="<?php echo $username_plaeholder; ?>" value="" type="text" class="form-control" id="erf_username" name="erf_username">
                </div>
                ...
                <div class="fb-text form-group">
					...
					<?php /* ?>
                    <input placeholder="<?php echo $attr['label_position'] == 'no-label' ? __('Password', 'erforms') : ''; ?>" type="password" value="" required="" class="form-control" id="erf_password" name="erf_password"> <?php */ ?>
                    <input placeholder="<?php _e('Enter password', 'erforms') ?>" type="password" value="" required="" class="form-control" id="erf_password" name="erf_password"> 
                </div>

                <div class="fb-text form-group rememberme">
                ...
                <?php if (!empty($attr['form_id']) && empty($form['login_and_register'])) : ?>
                    <div class="erf-account-switch">
                        ...
                        <a class="erf-show-lost-password"  href="javascript:void(0)" title="<?php _e('Lost/Forgot Password?', 'erforms') ?>"><?php _e('Forgot Password?', 'erforms') ?></a>
                    </div>
                <?php else: ?>
                    <div class="erf-account-switch">
                        <a class="erf-show-lost-password"  href="javascript:void(0)" title="<?php _e('Lost/Forgot Password?', 'erforms') ?>"><?php _e('Forgot Password?', 'erforms') ?></a>
