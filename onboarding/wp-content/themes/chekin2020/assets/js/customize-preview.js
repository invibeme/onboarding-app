/* global chekin2020BgColors, chekin2020PreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( chekin2020BgColors ).forEach( function( context ) {
					chekin2020GenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function chekin2020GenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'chekin2020-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#chekin2020-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( chekin2020PreviewEls[ context ] ) {
				_.each( chekin2020PreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		chekin2020GenerateColorA11yPreviewStyles( 'content' );
		chekin2020GenerateColorA11yPreviewStyles( 'header-footer' );
	} );
