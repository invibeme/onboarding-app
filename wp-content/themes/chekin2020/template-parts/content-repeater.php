...
<?php 
	...
	$custom_excerpt = substr(wp_kses(get_the_excerpt(),$tag_allow), 0, 250).'...'; 
	$cats = '';
	...
	if(has_post_thumbnail() && !empty(get_the_post_thumbnail_url($post, 'large'))){
		$post_thumbnail_url = get_the_post_thumbnail_url($post, 'large');
	}
	
	
	$output = '';
	//$output .= '<figure class="figure"><a href="'. get_the_permalink() .'" aria-label="Read more about '. get_the_title() .'"><img src="'.get_template_directory_uri().'/timthumb.php?src='. $post_thumbnail_url.'&amp;w=352&amp;h=221&amp;zc=1&amp;a=t" alt="" width="352" height="221" ></a></figure>';

	if ( has_post_thumbnail() ) :
		$output .= '<figure class="figure"><a href="'. get_the_permalink() .'" aria-label="Read more about '. get_the_title() .'">'.get_the_post_thumbnail($post, 'medium').'</a></figure>';
	endif;
	//$output .= '<p class="post-meta "><date class="date">'.get_the_date( 'F j, Y' ).'</date></p>';
	$output .= '<footer class="post-footer"><ul class="reset-list-style list-inline"><li><i class="icon icon-view"></i>'.do_shortcode('[views id="'.get_the_ID($post).'"]').'</li><li class="has-dropdown"><span class="has-children"><i class="icon icon-share"></i>'.__( 'Share', 'chekin2020' ).'</span>'.$share.'</li></ul></footer>';
	$output .= $cats; 
